...
from domain.workflow import (
    Workflow, WorkflowRun, StepResult, fill_template,
)
from infrastructure.anthropic_api.workflow_gateway import WorkflowGateway


def run_workflow(gateway: WorkflowGateway, wf: Workflow,
        ...
        if not ready:
            ...
            for s in pending:
                results.append(StepResult(
                    step_id=s.step_id, output="", latency_ms=0,
                    status="error", error=f"Dependency deadlock — remaining: {remaining_ids}",
                ))
            break
        for step in ready:
            pending.remove(step)
            ...
            try:
                output, ms = gateway.run_step(
                    step.model or wf.model, instruction, step.max_tokens,
                )
                ...
                results.append(StepResult(
                    step_id=step.step_id, output="", latency_ms=ms,
                    status="error", error=str(e),
                ))
                completed.add(step.step_id)

    return WorkflowRun(
        workflow=wf.name, results=results, variables=variables,

...
def gather_research(gateway: DeepResearchGateway, sq: SubQ, source_urls: list[str] | None = None) -> SubQ:
    ctx_parts = []
    ...
    raw = gateway._call(
        ...
        + (
            f"Source material:\\n{ctx}\\n\\n"
            if ctx
            else "(No sources — answer from knowledge; flag claims that need verification.)\\n\\n"
        )
...
def synthesize_research(gateway: DeepResearchGateway, topic: str, sqs: list[SubQ]) -> str:
    block = "\\n\\n".join(f"Q: {sq.question}\\n" + "\\n".join(f"- {f}" for f in sq.findings) for sq in sqs)
    return gateway._call(
        SYS_SYNTH,
...
def run_research(
    gateway: DeepResearchGateway, topic: str, depth: int = 4, source_urls: list[str] | None = None
) -> Report:

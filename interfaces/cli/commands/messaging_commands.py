...
__all__ = [
    "cmd_stream", "cmd_stream_tools", "cmd_structured", "cmd_structured_analyse",
    "cmd_structured_extract", "cmd_cite", "cmd_rag", "cmd_thinking",
    "cmd_count_tokens", "cmd_live", "ThinkingModeError",
]
...
def cmd_stream(prompt: str, api_key: str, model: str, system: str = None,
    ...
    result = service.stream_text(
        prompt, api_key, model, system=system, file_content=file_content,
        show_thinking=show_thinking, on_text=_print_text,
        on_thinking=on_thinking, on_thinking_start=on_thinking_start,
        on_thinking_stop=on_thinking_stop, on_usage=on_usage,
    )
    print()  # final newline
    if usage_box:
        print(f"\033[90m[tokens] in={usage_box.get('input_tokens', 0)}  "
              f"out={usage_box.get('output_tokens', 0)}\033[0m")
    return result


def cmd_stream_tools(prompt: str, tools: list, api_key: str, model: str,
                      system: str = None):
    ...
    result = service.stream_with_tools(
        prompt, tools, api_key, model, system=system, on_text=_print_text,
        on_tool_start=on_tool_start, on_tool_delta=on_tool_delta,
        on_tool_stop=on_tool_stop, on_refusal=on_refusal,
    )
    print()
...
def cmd_structured(prompt: str, api_key: str, model: str,
    ...
    outcome = service.generate_structured(prompt, api_key, model,
...
def cmd_thinking(prompt: str, api_key: str, model: str, budget: int,
                  effort: str, adaptive, show_thinking: bool,
    ...
    if stream:
        ...
        result = service.generate_thinking(
            prompt, api_key, model, budget, effort, adaptive, show_thinking, stream,
            system=system, display_omitted=display_omitted, legacy_budget=legacy_budget,
            on_text=_print_text, on_thinking=on_thinking,
            on_thinking_start=on_thinking_start, on_thinking_stop=on_thinking_stop,
        )
        print()
        return result
...
def cmd_count_tokens(prompt: str, api_key: str, model: str,
                      ...
                      budget: int = None):
    outcome = service.count_tokens(prompt, api_key, model, system=system,
                                    file_path=file_path, budget=budget)

...
- [x] Context #4 — Files & Documents ✅ **COMPLETE 2026-08-18** — `claude_files.py`
  ...
  `~/.zcoder/` on every construction, not lazily on first write;
  ...
  `--file-list` CLI run against `~/.zcoder/` afterward, not just
...
- [x] Context #5 — Sessions, Memory & Cache — **COMPLETE 2026-08-18**:
  ...
  *method* on the gateway class — is the second time this session
  ...
  actual `~/.zcoder/memory/` disk state (cleaned up after), and both
...
- [x] **Context #7 (Cost, Metrics & Eval) — COMPLETE 2026-08-19.**
  ...
  live `~/.zcoder/metrics.jsonl` on the machine running this
  ...
  cleaned up (`rm -rf ~/.zcoder`) and re-verified both the fixed test
  ...
  **945/945 suite green (879 baseline + 66 new — the historical "888"
  ...
  `~/.zcoder/` state for all of `--cost-summary`, `--metrics-show`,
...
| Files API → GA (Aug 19–20) | `files-api-2025-04-14` beta header no longer required; GA adds expiration + pagination. Old beta-header requests keep working | `domain/skills_api.py` still sends the beta header — harmless (back-compat honored); optional follow-up to drop it and adopt GA response format | ✅ **CLOSED v1.44.0** — beta header removed; GA `expires_at` + pagination (`next_page`, legacy fallback) adopted in `files_gateway.py` |
| Agent Skills / Skills API → GA (Aug 19–20) | Skills beta header no longer required on `/v1/skills` or Messages `container` usage; old requests keep working | Same disposition as Files API — header still sent, still accepted | ✅ **CLOSED v1.44.0** — skills beta header removed from both gateways; only code-execution beta remains |
| Computer use tool → GA (`computer_toolset_20260801`) | No beta header; batch actions; zoom default; per-member configs | ZCoder uses the earlier beta toolset shape via `models_gateway.py` — still functional; migration to GA toolset is a feature-cycle item | ✅ **CLOSED v1.44.0** — GA toolset is the new default (batch_actions/zoom/configs descriptor, no beta header); legacy shape opt-in via `toolset="legacy"`; model-gated {fable5,mythos5,opus5,sonnet5,opus4.8} |

...
def dispatch(args):
    ...
    if args.plugin_marketplace_add:
        from interfaces.cli.commands.plugins_commands import cmd_plugin_marketplace_add

        cmd_plugin_marketplace_add(args.plugin_marketplace_add, args.plugin_marketplace_name)
        return
    if args.plugin_marketplace_list:
        from interfaces.cli.commands.plugins_commands import cmd_plugin_marketplace_list

        cmd_plugin_marketplace_list()
        return
    if args.plugin_marketplace_remove:
        from interfaces.cli.commands.plugins_commands import cmd_plugin_marketplace_remove

        cmd_plugin_marketplace_remove(args.plugin_marketplace_remove)
        return
    if args.plugin_install:
        from interfaces.cli.commands.plugins_commands import cmd_plugin_install

        cmd_plugin_install(args.plugin_install)
        return
    if args.plugin_dir:
        from interfaces.cli.commands.plugins_commands import cmd_plugin_install_dir

        cmd_plugin_install_dir(args.plugin_dir)
        return
    if args.plugin_uninstall:
        from interfaces.cli.commands.plugins_commands import cmd_plugin_uninstall
    ...
    if args.plugin_list:
        ...
        return
    if args.plugin_info:
        from interfaces.cli.commands.plugins_commands import cmd_plugin_info

        cmd_plugin_info(args.plugin_info)
        return
    if args.plugin_enable:
        from interfaces.cli.commands.plugins_commands import cmd_plugin_enable

        cmd_plugin_enable(args.plugin_enable)
        return
    if args.plugin_disable:
        from interfaces.cli.commands.plugins_commands import cmd_plugin_disable

        cmd_plugin_disable(args.plugin_disable)
        return
    if args.plugin_validate:
        from interfaces.cli.commands.plugins_commands import cmd_plugin_validate
    ...
    if args.settings_show:
        from interfaces.cli.commands.settings_commands import cmd_settings_show
    ...
    if args.skills_list:
        from interfaces.cli.commands.skills_api_commands import cmd_skills_list
    ...
    if (
        ...
        if args.admin_create_key:
            from interfaces.cli.commands.admin_commands import cmd_admin_create_key
        ...
        if args.usage_report:
            from interfaces.cli.commands.admin_commands import cmd_usage_report
            ...
            return
        if args.cost_report:
            from interfaces.cli.commands.admin_commands import cmd_cost_report
        ...
        if args.admin_list_keys:
            from interfaces.cli.commands.admin_commands import cmd_admin_list_keys

            cmd_admin_list_keys(admin_key)
            return
        if args.admin_revoke_key:
            from interfaces.cli.commands.admin_commands import cmd_admin_revoke_key
        ...
        if args.spend_limit_get:
            from interfaces.cli.commands.admin_commands import cmd_spend_limit_get
        ...
        if args.spend_limit_request_approve:
            from interfaces.cli.commands.admin_commands import cmd_spend_limit_request_approve

            cmd_spend_limit_request_approve(args.spend_limit_request_approve, admin_key)
            return
        if args.spend_limit_request_deny:
            from interfaces.cli.commands.admin_commands import cmd_spend_limit_request_deny

            cmd_spend_limit_request_deny(args.spend_limit_request_deny, admin_key)
            return
        if args.rate_limits_workspace:
            from interfaces.cli.commands.admin_commands import cmd_rate_limits_workspace

            cmd_rate_limits_workspace(args.rate_limits_workspace, admin_key)
            return
        if args.rate_limits:
            from interfaces.cli.commands.admin_commands import cmd_rate_limits
    ...
    if (
        ...
        if args.members_list:
            from interfaces.cli.commands.admin_commands import cmd_members_list

            cmd_members_list(admin_key, email=args.members_email or None)
            return
        if args.member_get:
            from interfaces.cli.commands.admin_commands import cmd_member_get

            cmd_member_get(args.member_get, admin_key)
            return
        if args.member_role_set:
            from interfaces.cli.commands.admin_commands import cmd_member_role_set

            user_id, role = args.member_role_set
            cmd_member_role_set(user_id, role, admin_key)
            return
        if args.member_remove:
            from interfaces.cli.commands.admin_commands import cmd_member_remove

            cmd_member_remove(args.member_remove, admin_key)
            return
        if args.invite_create:
            from interfaces.cli.commands.admin_commands import cmd_invite_create
        ...
        if args.invites_list:
            from interfaces.cli.commands.admin_commands import cmd_invites_list

            cmd_invites_list(admin_key)
            return
        if args.invite_withdraw:
            from interfaces.cli.commands.admin_commands import cmd_invite_withdraw

            cmd_invite_withdraw(args.invite_withdraw, admin_key)
            return
        if args.groups_list:
            from interfaces.cli.commands.admin_commands import cmd_groups_list

            cmd_groups_list(admin_key)
            return
        if args.group_create:
            from interfaces.cli.commands.admin_commands import cmd_group_create

            cmd_group_create(args.group_create, admin_key)
            return
        if args.group_delete:
            from interfaces.cli.commands.admin_commands import cmd_group_delete

            cmd_group_delete(args.group_delete, admin_key)
            return
        if args.group_members_list:
            from interfaces.cli.commands.admin_commands import cmd_group_members_list

            cmd_group_members_list(args.group_members_list, admin_key)
            return
        if args.group_member_add:
            from interfaces.cli.commands.admin_commands import cmd_group_member_add

            group_id, user_id = args.group_member_add
            cmd_group_member_add(group_id, user_id, admin_key)
            return
        if args.group_member_remove:
            from interfaces.cli.commands.admin_commands import cmd_group_member_remove

            group_id, user_id = args.group_member_remove
            cmd_group_member_remove(group_id, user_id, admin_key)
            return
        if args.roles_list:
            from interfaces.cli.commands.admin_commands import cmd_roles_list

            cmd_roles_list(admin_key)
            return
        if args.role_permissions:
            from interfaces.cli.commands.admin_commands import cmd_role_permissions_list
    ...
    if (
        ...
        if args.wif_status:
            from interfaces.cli.commands.wif_commands import cmd_wif_status

            cmd_wif_status()
            return
        if args.wif_exchange_token:
            from interfaces.cli.commands.wif_commands import cmd_wif_exchange_token
        ...
        if args.wif_create_service_account:
            from interfaces.cli.commands.wif_commands import cmd_wif_create_service_account

            cmd_wif_create_service_account(args.wif_create_service_account, org_admin_token)
            return
        if args.wif_list_service_accounts:
            from interfaces.cli.commands.wif_commands import cmd_wif_list_service_accounts

            cmd_wif_list_service_accounts(org_admin_token)
            return
        if args.wif_create_issuer:
            from interfaces.cli.commands.wif_commands import cmd_wif_create_issuer
        ...
        if args.wif_list_issuers:
            from interfaces.cli.commands.wif_commands import cmd_wif_list_issuers

            cmd_wif_list_issuers(org_admin_token)
            return
        if args.wif_create_rule:
            from interfaces.cli.commands.wif_commands import cmd_wif_create_rule
        ...
        if args.wif_list_rules:
            from interfaces.cli.commands.wif_commands import cmd_wif_list_rules
    ...
    if any(_compliance_flags):
        ...
        if args.compliance_activities:
            from interfaces.cli.commands.compliance_commands import cmd_compliance_activities
        ...
        if args.compliance_chats_list:
            from interfaces.cli.commands.compliance_commands import cmd_compliance_chats_list
        ...
        if args.compliance_chat_messages:
            from interfaces.cli.commands.compliance_commands import cmd_compliance_chat_messages

            cmd_compliance_chat_messages(compliance_key, args.compliance_chat_messages)
            return
        if args.compliance_chat_delete:
            from interfaces.cli.commands.compliance_commands import cmd_compliance_chat_delete
        ...
        if args.compliance_file_download:
            from interfaces.cli.commands.compliance_commands import cmd_compliance_file_download
        ...
        if args.compliance_file_delete:
            from interfaces.cli.commands.compliance_commands import cmd_compliance_file_delete
        ...
        if args.compliance_projects_list:
            from interfaces.cli.commands.compliance_commands import cmd_compliance_projects_list

            cmd_compliance_projects_list(compliance_key)
            return
        if args.compliance_project_info:
            from interfaces.cli.commands.compliance_commands import cmd_compliance_project_info

            cmd_compliance_project_info(compliance_key, args.compliance_project_info)
            return
        if args.compliance_project_attachments:
            from interfaces.cli.commands.compliance_commands import cmd_compliance_project_attachments

            cmd_compliance_project_attachments(compliance_key, args.compliance_project_attachments)
            return
        if args.compliance_project_delete:
            from interfaces.cli.commands.compliance_commands import cmd_compliance_project_delete
        ...
        if args.compliance_orgs_list:
            from interfaces.cli.commands.compliance_commands import cmd_compliance_orgs_list

            cmd_compliance_orgs_list(compliance_key)
            return
        if args.compliance_org_users:
            from interfaces.cli.commands.compliance_commands import cmd_compliance_org_users

            cmd_compliance_org_users(compliance_key, args.compliance_org_users)
            return
        if args.compliance_org_roles:
            from interfaces.cli.commands.compliance_commands import cmd_compliance_org_roles

            cmd_compliance_org_roles(compliance_key, args.compliance_org_roles)
            return
        if args.compliance_org_settings:
            from interfaces.cli.commands.compliance_commands import cmd_compliance_org_settings

            cmd_compliance_org_settings(compliance_key, args.compliance_org_settings)
            return
        if args.compliance_groups_list:
            from interfaces.cli.commands.compliance_commands import cmd_compliance_groups_list

            cmd_compliance_groups_list(compliance_key)
            return
        if args.compliance_group_members:
            from interfaces.cli.commands.compliance_commands import cmd_compliance_group_members
        ...
        if args.compliance_local_session_get:
            from interfaces.cli.commands.compliance_commands import cmd_compliance_local_session_get
        ...
        if args.compliance_local_session_messages:
            from interfaces.cli.commands.compliance_commands import cmd_compliance_local_session_messages
        ...
        if args.compliance_remote_sessions_list:
            from interfaces.cli.commands.compliance_commands import cmd_compliance_remote_sessions_list
    ...
    if args.check_deprecated:
        from interfaces.cli.commands.model_commands import cmd_check_deprecated

        cmd_check_deprecated(args.check_deprecated)
        return
    if args.upgrade_all:
        from interfaces.cli.commands.model_commands import cmd_upgrade_all
    ...
    if args.list_server_tools:
        from interfaces.cli.commands.tools_commands import cmd_list_server_tools
    ...
    if args.agent_list_sessions:
        from interfaces.cli.commands.agent_commands import cmd_agent_list_sessions

        cmd_agent_list_sessions()
        return
    if args.list_tool_presets:
        from interfaces.cli.commands.agent_commands import cmd_list_tool_presets

        cmd_list_tool_presets()
        return
    if args.code_agent_list_sessions:
        from interfaces.cli.commands.code_agent_loop_commands import cmd_code_list_sessions

        cmd_code_list_sessions()
        return
    if args.code_agent_list_tools:
        from interfaces.cli.commands.code_agent_loop_commands import cmd_code_list_tools
    ...
    if args.memory_add:
        from interfaces.cli.commands.memory_commands import cmd_memory_add
    ...
    if args.memory_recall:
        from interfaces.cli.commands.memory_commands import cmd_memory_recall

        cmd_memory_recall(args.memory_recall, args.memory_ns)
        return
    if args.memory_forget:
        from interfaces.cli.commands.memory_commands import cmd_memory_forget

        cmd_memory_forget(args.memory_forget, args.memory_ns)
        return
    if args.memory_stats:
        from interfaces.cli.commands.memory_commands import cmd_memory_stats

        cmd_memory_stats(args.memory_ns)
        return
    if args.memory_retention:
        from interfaces.cli.commands.memory_commands import cmd_memory_retention

        cmd_memory_retention(args.memory_ns)
        return
    if args.sessions_list:
        from interfaces.cli.commands.sessions_commands import cmd_sessions_list

        cmd_sessions_list()
        return
    if args.session_show:
        from interfaces.cli.commands.sessions_commands import cmd_session_show

        cmd_session_show(args.session_show)
        return
    if args.checkpoint_list:
        from interfaces.cli.commands.sessions_commands import cmd_checkpoint_list

        cmd_checkpoint_list(args.checkpoint_list)
        return
    if args.away_summary:
        from interfaces.cli.commands.sessions_commands import cmd_away_summary

        cmd_away_summary(args.away_summary)
        return
    if args.rag_index and args.rag_folder:
        from interfaces.cli.commands.tools_commands import cmd_rag_index

        cmd_rag_index(args.rag_index, args.rag_folder)
        return
    if args.rag_list:
        from interfaces.cli.commands.tools_commands import cmd_rag_list

        cmd_rag_list()
        return
    if args.eval_list:
        from interfaces.cli.commands.observability_commands import cmd_eval_list

        cmd_eval_list()
        return
    if args.eval_scaffold:
        from interfaces.cli.commands.observability_commands import cmd_eval_scaffold

        cmd_eval_scaffold(args.eval_scaffold)
        return
    if args.cost_summary:
        from interfaces.cli.commands.observability_commands import cmd_cost_summary

        cmd_cost_summary()
        return
    if args.cost_reset:
        from interfaces.cli.commands.observability_commands import cmd_cost_reset

        cmd_cost_reset()
        return
    if args.metrics_show:
        from interfaces.cli.commands.observability_commands import cmd_metrics_show

        cmd_metrics_show(today_only=args.metrics_today, model_filter=args.metrics_model or None)
        return
    if args.metrics_clear:
        from interfaces.cli.commands.observability_commands import cmd_metrics_clear
    ...
    if args.metrics_export:
        ...
        return
    if args.obs_latency:
        from interfaces.cli.commands.observability_commands import cmd_obs_latency

        cmd_obs_latency(args.obs_hours)
        return
    if args.obs_tail is not None:
        from interfaces.cli.commands.observability_commands import cmd_obs_tail

        cmd_obs_tail(args.obs_tail)
        return
    if args.obs_clear:
        from interfaces.cli.commands.observability_commands import cmd_obs_clear

        cmd_obs_clear()
        return
    if args.workflow_scaffold:
        from interfaces.cli.commands.workflow_commands import cmd_workflow_scaffold

        cmd_workflow_scaffold(args.workflow_scaffold)
        return
    if args.hooks_add:
        from interfaces.cli.commands.code_agent_commands import cmd_hooks_add

        cmd_hooks_add(args.hooks_add[0], args.hooks_add[1], args.hook_tool_match)
        return
    if args.hooks_list:
        from interfaces.cli.commands.code_agent_commands import cmd_hooks_list

        cmd_hooks_list()
        return
    if args.hooks_remove is not None:
        from interfaces.cli.commands.code_agent_commands import cmd_hooks_remove

        cmd_hooks_remove(args.hooks_remove)
        return
    if args.perms_list:
        from interfaces.cli.commands.code_agent_commands import cmd_perms_list

        cmd_perms_list()
        return
    if args.perms_add:
        from interfaces.cli.commands.code_agent_commands import cmd_perms_add
    ...
    if args.excel is not None:
        from interfaces.cli.commands.excel_commands import cmd_excel_chat
    ...
    if args.pptx is not None:
        from interfaces.cli.commands.pptx_commands import cmd_pptx_chat
    ...
    if args.list_models:
        from interfaces.cli.commands.model_commands import cmd_list_models

        cmd_list_models(key, include_legacy=getattr(args, "list_models_legacy", False))
        return
    if args.model_info:
        from interfaces.cli.commands.model_commands import cmd_model_info
    ...
    if args.rag_query:
        from interfaces.cli.commands.tools_commands import cmd_rag_query
    ...
    if args.eval_run:
        from interfaces.cli.commands.observability_commands import cmd_eval_run

        cmd_eval_run(args.eval_run, key, model, threshold=args.eval_threshold, output=args.output)
        return
    if args.eval_compare:
        from interfaces.cli.commands.observability_commands import cmd_eval_compare
    ...
    # ── Git Integration ──
    if args.git_commit:
        from interfaces.cli.commands.devtools_commands import cmd_git_commit

        cmd_git_commit(key, model, style=args.git_commit_style, write=args.git_commit_write)
        return
    if args.git_pr:
        from interfaces.cli.commands.devtools_commands import cmd_git_pr

        cmd_git_pr(args.git_pr[0], args.git_pr[1], key, model)
        return
    if args.git_changelog:
        from interfaces.cli.commands.devtools_commands import cmd_git_changelog

        cmd_git_changelog(args.git_changelog, key, model, output=args.output)
        return
    if args.git_review:
        from interfaces.cli.commands.devtools_commands import cmd_git_review

        cmd_git_review(key, model)
        return
    if args.git_blame_explain:
        from interfaces.cli.commands.devtools_commands import cmd_git_blame_explain
    ...
    # ── Multi-Agent Router ──
    if args.route_list:
        from interfaces.cli.commands.code_agent_commands import cmd_route_list

        cmd_route_list()
        return
    if args.route:
        from interfaces.cli.commands.code_agent_commands import cmd_route
    ...
    if args.prompt_lib_list:
        from interfaces.cli.commands.prompt_optimizer_commands import cmd_prompt_lib_list

        cmd_prompt_lib_list()
        return
    if args.prompt_lib_get:
        from domain.prompt_optimizer import lib_get_entry as lib_get
    ...
    if args.prompt_lib_add:
        from domain.prompt_optimizer import lib_add_entry as lib_add
        ...
        return
    if args.ab_test:
        from interfaces.cli.commands.prompt_optimizer_commands import cmd_ab_test
    ...
    if args.score_prompt:
        from interfaces.cli.commands.prompt_optimizer_commands import cmd_score

        cmd_score(args.score_prompt, key, model)
        return
    if args.prompt_optimize:
        from interfaces.cli.commands.prompt_optimizer_commands import cmd_optimize
    ...
    if args.optimized:
        from interfaces.cli.commands.observability_commands import cmd_optimized
    ...
    if args.obs_errors:
        from interfaces.cli.commands.observability_commands import cmd_obs_errors
    ...
    if args.workflow_run:
        from interfaces.cli.commands.workflow_commands import cmd_workflow_run
    ...
    # ── Plan Mode ──
    if args.plan:
        from interfaces.cli.commands.code_agent_commands import cmd_plan
    ...
    if args.thinking or args.adaptive or args.effort_legacy_budget:
        from domain.messaging import ThinkingModeError
        ...
        return
    if args.stream:
        from interfaces.cli.commands.messaging_commands import cmd_stream
    ...
    if args.web_search or args.web_fetch:
        from interfaces.cli.commands.tools_commands import cmd_web_search
        ...
        return
    if args.fetch_url:
        from interfaces.cli.commands.tools_commands import cmd_fetch_url

        cmd_fetch_url(args.fetch_url, args.prompt or "", key, model)
        return
    if args.vision:
        from interfaces.cli.commands.tools_commands import cmd_vision
        ...
        return
    if args.vision_pdf:
        from interfaces.cli.commands.tools_commands import cmd_vision_pdf

        cmd_vision_pdf(args.vision_pdf, args.prompt or "", key, model)
        return
    if args.vision_url:
        from interfaces.cli.commands.tools_commands import cmd_vision_url

        cmd_vision_url(args.vision_url, args.prompt or "", key, model)
        return
    if args.vision_compare:
        from interfaces.cli.commands.tools_commands import cmd_vision_compare

        cmd_vision_compare(args.vision_compare, args.prompt or "", key, model)
        return
    if args.vision_ocr:
        from interfaces.cli.commands.tools_commands import cmd_vision_ocr

        cmd_vision_ocr(args.vision_ocr, key, model)
        return
    if args.batch_submit:
        from interfaces.cli.commands.batch_commands import cmd_batch_submit

        cmd_batch_submit(args.batch_submit, key, model, use_300k_output=args.batch_300k_output)
        return
    if args.batch_status:
        from interfaces.cli.commands.batch_commands import cmd_batch_status

        cmd_batch_status(args.batch_status, key)
        return
    if args.batch_results:
        from interfaces.cli.commands.batch_commands import cmd_batch_results

        cmd_batch_results(args.batch_results, key, save_to=args.output or None)
        return
    if args.batch_cancel:
        from interfaces.cli.commands.batch_commands import cmd_batch_cancel

        cmd_batch_cancel(args.batch_cancel, key)
        return
    if args.batch_list:
        from interfaces.cli.commands.batch_commands import cmd_batch_list

        cmd_batch_list(key)
        return
    if args.batch_generate > 0:
        from interfaces.cli.commands.batch_commands import cmd_batch_generate

        cmd_batch_generate(args.prompt or "", args.batch_generate, key, model, wait=args.batch_wait)
        return
    if args.cache_warm:
        from interfaces.cli.commands.cache_commands import cmd_cache_warm
    ...
    if args.cache_multi_turn:
        from interfaces.cli.commands.cache_commands import cmd_cache_multi_turn
        ...
        return
    if args.cache:
        from interfaces.cli.commands.cache_commands import cmd_cache_generate
        ...
        return
    if args.tool_agent:
        from interfaces.cli.commands.tools_commands import cmd_tool_agent

        cmd_tool_agent(args.prompt or "", key, model, max_turns=args.max_turns)
        return
    if args.server_tool:
        from interfaces.cli.commands.tools_commands import cmd_server_tool
    ...
    if args.memory_agent:
        from interfaces.cli.commands.tools_commands import cmd_memory_agent
    ...
    if args.advisor:
        ...
        return
    if args.embed:
        from interfaces.cli.commands.tools_commands import cmd_embed

        cmd_embed(args.embed, model=args.embed_model, input_type=args.embed_input_type)
        return
    if args.embed_file:
        from interfaces.cli.commands.tools_commands import cmd_embed_file

        cmd_embed_file(args.embed_file, model=args.embed_model, input_type=args.embed_input_type)
        return
    if args.embed_similarity:
        from interfaces.cli.commands.tools_commands import cmd_embed_similarity
    ...
    if args.stream_tools:
        ...
        from interfaces.cli.commands.messaging_commands import cmd_stream_tools
        ...
        return
    if args.structured:
        from interfaces.cli.commands.messaging_commands import cmd_structured
    ...
    if args.structured_analyse:
        from interfaces.cli.commands.messaging_commands import cmd_structured_analyse

        cmd_structured_analyse(args.structured_analyse, key, model)
        return
    if args.structured_extract:
        from interfaces.cli.commands.messaging_commands import cmd_structured_extract

        cmd_structured_extract(args.structured_extract, args.schema, key, model)
        return
    if args.file_upload:
        from interfaces.cli.commands.files_commands import cmd_file_upload

        cmd_file_upload(args.file_upload, key, model)
        return
    if args.file_list:
        from interfaces.cli.commands.files_commands import cmd_file_list

        cmd_file_list(key, model)
        return
    if args.file_delete:
        from interfaces.cli.commands.files_commands import cmd_file_delete

        cmd_file_delete(args.file_delete, key)
        return
    if args.file_ask:
        from interfaces.cli.commands.files_commands import cmd_file_ask

        cmd_file_ask(args.file_ask, args.prompt or "Summarise.", key, model, media_type=args.file_media_type)
        return
    if args.file_download:
        from interfaces.cli.commands.files_commands import cmd_file_download
    ...
    if args.code_exec:
        ...
        return
    if args.code_debug:
        from interfaces.cli.commands.code_agent_commands import cmd_code_debug
    ...
    if args.count_tokens:
        ...
        return
    if args.cite:
        from interfaces.cli.commands.messaging_commands import cmd_cite

        cmd_cite(args.prompt or "", args.cite, key, model)
        return
    if args.rag:
        from interfaces.cli.commands.messaging_commands import cmd_rag

        cmd_rag(args.prompt or "", args.rag, key, model, pattern=args.rag_pattern)
        return
    if args.computer_use:
        from interfaces.cli.commands.model_commands import cmd_computer_use

        cmd_computer_use(args.computer_use, key, model)
        return
    if args.interleaved_thinking:
        from interfaces.cli.commands.model_commands import cmd_adaptive_thinking
    ...
    if args.agent_session or args.agent_orchestrate:
        from interfaces.cli.commands.agent_commands import cmd_agent_chat, cmd_agent_orchestrate
        ...
        return
    if args.agent_dream:
        from interfaces.cli.commands.agent_commands import cmd_agent_dream
    ...
    if args.agent_dream_get:
        from interfaces.cli.commands.agent_commands import cmd_agent_dream_get

        cmd_agent_dream_get(args.agent_dream_get, key)
        return
    if args.agent_dream_cancel:
        from interfaces.cli.commands.agent_commands import cmd_agent_dream_cancel

        cmd_agent_dream_cancel(args.agent_dream_cancel, key)
        return
    if args.agent_dream_archive:
        from interfaces.cli.commands.agent_commands import cmd_agent_dream_archive

        cmd_agent_dream_archive(args.agent_dream_archive, key)
        return
    if args.agent_dream_list:
        from interfaces.cli.commands.agent_commands import cmd_agent_dream_list
    ...
    if args.agent_webhook_register:
        from interfaces.cli.commands.agent_commands import cmd_agent_webhook_register
    ...
    if args.agent_vault_create:
        from interfaces.cli.commands.agent_commands import cmd_agent_vault_create
    ...
    if args.agent_vault_add_credential:
        from interfaces.cli.commands.agent_commands import cmd_agent_vault_add_credential
    ...
    if args.agent_vault_list:
        from interfaces.cli.commands.agent_commands import cmd_agent_vault_list

        cmd_agent_vault_list(key)
        return
    if args.agent_schedule_create:
        from interfaces.cli.commands.agent_commands import cmd_agent_schedule_create
    ...
    if args.agent_schedule_list:
        from interfaces.cli.commands.agent_commands import cmd_agent_schedule_list

        cmd_agent_schedule_list(key)
        return
    if args.agent_schedule_cancel:
        from interfaces.cli.commands.agent_commands import cmd_agent_schedule_cancel

        cmd_agent_schedule_cancel(args.agent_schedule_cancel, key)
        return
    if args.agent_review_multiagent:
        from interfaces.cli.commands.agent_commands import cmd_agent_review_multiagent
    ...
    if args.agent_outcome_rubric_upload:
        from interfaces.cli.commands.agent_commands import cmd_agent_outcome_rubric_upload

        cmd_agent_outcome_rubric_upload(args.agent_outcome_rubric_upload, key, model)
        return
    if args.agent_env_self_hosted:
        from interfaces.cli.commands.agent_commands import cmd_agent_env_self_hosted_create

        cmd_agent_env_self_hosted_create(args.agent_env_self_hosted, key)
        return
    if args.agent_env_work_stats:
        from interfaces.cli.commands.agent_commands import cmd_agent_env_work_stats
    ...
    if args.agent_managed_run:
        ...
        from interfaces.cli.commands.agent_commands import cmd_managed_agent_run
    ...
    if args.agent_create:
        from interfaces.cli.commands.agent_commands import cmd_agent_create
        ...
        return
    if args.agent_get:
        from interfaces.cli.commands.agent_commands import cmd_agent_get

        cmd_agent_get(args.agent_get, key, version=args.agent_get_version)
        return
    if args.agent_list:
        from interfaces.cli.commands.agent_commands import cmd_agent_list

        cmd_agent_list(key, limit=args.agent_list_limit)
        return
    if args.agent_update:
        from interfaces.cli.commands.agent_commands import cmd_agent_update
    ...
    if args.agent_session_get:
        from interfaces.cli.commands.agent_commands import cmd_agent_session_get

        cmd_agent_session_get(args.agent_session_get, key)
        return
    if args.agent_session_budget_set:
        from interfaces.cli.commands.agent_commands import cmd_agent_session_budget_set
    ...
    if args.agent_session_budget_remove:
        from interfaces.cli.commands.agent_commands import cmd_agent_session_budget_remove

        cmd_agent_session_budget_remove(args.agent_session_budget_remove, key)
        return
    if args.agent_memory_store_create:
        from interfaces.cli.commands.agent_commands import cmd_agent_memory_store_create
    ...
    if args.agent_memory_list:
        from interfaces.cli.commands.agent_commands import cmd_agent_memory_list
    ...
    if args.agent_memory_stores_list:
        from interfaces.cli.commands.agent_commands import cmd_agent_memory_stores_list
    ...
    if args.agent_memory_store_archive:
        from interfaces.cli.commands.agent_commands import cmd_agent_memory_store_archive

        cmd_agent_memory_store_archive(args.agent_memory_store_archive, key)
        return
    if args.agent_memory_store_delete:
        from interfaces.cli.commands.agent_commands import cmd_agent_memory_store_delete
    ...
    if args.agent_memory_get:
        from interfaces.cli.commands.agent_commands import cmd_agent_memory_get
    ...
    if args.agent_memory_create:
        from interfaces.cli.commands.agent_commands import cmd_agent_memory_create
    ...
    if args.agent_memory_update:
        from interfaces.cli.commands.agent_commands import cmd_agent_memory_update
    ...
    if args.agent_memory_delete:
        from interfaces.cli.commands.agent_commands import cmd_agent_memory_delete
    ...
    if args.code_agent_mcp_tunnel:
        from interfaces.cli.commands.agent_commands import cmd_mcp_tunnel_open
    ...
    if args.code_agent_subagent:
        from interfaces.cli.commands.code_agent_loop_commands import cmd_code_subagent

        cmd_code_subagent(args.code_agent_subagent, key, model, cwd=args.code_agent_cwd)
        return
    if args.code_agent_todo:
        from interfaces.cli.commands.code_agent_loop_commands import cmd_code_todo

        cmd_code_todo(args.code_agent_todo, key, model)
        return
    if args.code_agent_slash:
        from interfaces.cli.commands.code_agent_loop_commands import cmd_code_slash

        cmd_code_slash(args.code_agent_slash, key, model, cwd=args.code_agent_cwd, prompt=args.prompt or "")
        return
    if args.code_agent_cost:
        from interfaces.cli.commands.code_agent_loop_commands import cmd_code_cost

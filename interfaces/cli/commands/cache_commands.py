...
def cmd_cache_generate(prompt: str, api_key: str, model: str, system: str = None,
                       docs: list = None, ttl: str = "5m",
                       show_stats: bool = True, diagnose: bool = False) -> str:
    print(f"\033[94mℹ Prompt caching enabled (TTL={ttl})\033[0m\n")
    result, stats = service.generate(prompt, api_key, model, system=system,
                                      docs=docs, ttl=ttl, diagnose=diagnose)
    print(result)
    if show_stats:
...
def cmd_cache_multi_turn(turns: list, api_key: str, model: str,
    ...
    try:
        responses, stats = service.multi_turn(turns, api_key, model, system=system,
                                               ttl=ttl, mid_system=mid_system,
...
def cmd_cache_warm(api_key: str, model: str, system: str = None,
                   doc_files: list = None, ttl: str = "5m"):
    print(f"\033[94mℹ Pre-warming cache (TTL={ttl})…\033[0m")
    usage, stats, errors = service.warm(api_key, model, system=system,
                                         doc_files=doc_files, ttl=ttl)
    for f, err in errors:

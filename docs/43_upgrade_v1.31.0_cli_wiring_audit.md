...
against `platform.claude.com/docs` — did we send the right request shape,
handle the right response fields, use the current beta header. This one
checked something else: for every `claude_*.py` module, is every
`cmd_*` function it defines actually reachable from `main.py`? A module
can be a perfectly correct, fully-tested implementation of some Claude
...
false negative for `claude_prompt_optimizer.cmd_optimize`, because
`main.py` contains the *unrelated* `cmd_optimized` (from
`claude_cost_optimizer.py`, a different module, different feature) and
...
|---|---|---|
| `claude_github.py` | `cmd_gh_review_pr`, `cmd_gh_triage`, `cmd_gh_commits`, `cmd_gh_pr_description` | `--gh-review-pr`, `--gh-triage-issues`, `--gh-summarise-commits`, `--gh-pr-description`, `--gh-token`, `--gh-max-items` |
| `claude_router.py` | `cmd_route`, `cmd_route_list` | `--route`, `--route-explain`, `--route-parallel`, `--route-list` |
| `claude_prompt_optimizer.py` | `cmd_optimize`, `cmd_score`, `cmd_ab_test`, `cmd_prompt_lib_list` (+ `lib_add`/`lib_get` used directly, no `cmd_` wrapper existed for those two) | `--optimize`, `--score-prompt`, `--ab-test`, `--ab-prompt-b`, `--ab-task`, `--prompt-lib-add`, `--prompt-lib-list`, `--prompt-lib-get` |
| `claude_metrics.py` | `cmd_metrics_show`, `cmd_metrics_clear`, `cmd_metrics_export` | `--metrics-show`, `--metrics-today`, `--metrics-model`, `--metrics-clear`, `--metrics-export` |
...
`claude_prompt_optimizer.py` docstring documents `--ab-test ... With
...
## One thing found and *not* wired: `claude_evals.py`

`claude_evals.py` (plural) defines `cmd_eval`, also undocumented in
`main.py`, also carrying a `CLI flags:` docstring describing `--eval`,
`--eval-create`, etc. But `claude_eval.py` (singular) already covers the
...
Wiring `claude_evals.cmd_eval` too would mean two different `--eval`-
...
by someone who didn't know the history. `claude_evals.py` is a
...
`--route-add-agent`, mentioned in `claude_router.py`'s own docstring,
...
parametrized test that walks every `claude_*.py` file, collects its
...
`claude_evals.cmd_eval` exception is recorded with its reasoning inline,

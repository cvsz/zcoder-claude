...
def urlopen_json_with_headers(req: urllib.request.Request, timeout: float) -> tuple[dict, dict]:
    """Like `urlopen_json`, but also returns the response headers.

    Added 2026-08-14: `urlopen_json` alone discards response headers, so
    nothing in the codebase could read the `anthropic-workspace-id` header
    the Claude API added on 2026-08-11 (the ID of the workspace an API
    key/token resolved to, including the org's Default Workspace — see
    platform.claude.com/docs/en/manage-claude/workspaces#identify-the-
    workspace-behind-an-api-response), or the older
    `anthropic-organization-id` header. Existing call sites keep using
    plain `urlopen_json` unchanged; use this variant where the header is
    actually needed (see `claude_admin_api.get_response_workspace_id()`)."""
    try:
        with urllib.request.urlopen(req, timeout=timeout) as r:

...
def test_log_spend_caps_at_5000_entries(tmp_path, monkeypatch):
    log = tmp_path / "cost_log.json"
    monkeypatch.setattr(store, "SPEND_LOG", log)
    log.write_text(json.dumps([{"ts": "x", "model": "m", "in_tok": 1,
                                "out_tok": 1, "cost_usd": 0.0, "prompt": "p"}] * 5000))
...
def test_save_and_load_eval_run_summaries(tmp_path, monkeypatch):
    ...
    run = EvalRun(run_id="abc12345", model="claude-sonnet-5", cases=1, passed=1,
                  avg_score=1.0, avg_latency_ms=50.0,
                  results=[EvalResult(case_id="c1", prompt="p", expected="e",
                                      actual="a", score=1.0, passed=True,
                                      latency_ms=50, model="claude-sonnet-5")])
...
def test_write_eval_first_result_json_writes_only_first_result(tmp_path):
    ...
    run = EvalRun(run_id="abc12345", model="claude-sonnet-5", cases=2, passed=1,
                  avg_score=0.5, avg_latency_ms=100.0,
                  results=[
                      EvalResult(case_id="c1", prompt="p", expected="e", actual="a",
                                score=1.0, passed=True, latency_ms=50, model="m"),
                      EvalResult(case_id="c2", prompt="p", expected="e", actual="a",
                                score=0.0, passed=False, latency_ms=150, model="m"),
                  ])
...
def test_write_eval_first_result_json_empty_results(tmp_path):
    out = tmp_path / "output.json"
    run = EvalRun(run_id="abc", model="m", cases=0, passed=0,
                  avg_score=0, avg_latency_ms=0, results=[])
    store.write_eval_first_result_json(str(out), run)

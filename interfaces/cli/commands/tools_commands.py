...
def cmd_tool_agent(prompt: str, api_key: str, model: str, system: str = None, max_turns: int = 10):
    ...
    result = service.run_tool_agent(
        prompt, api_key, model, system=system, max_turns=max_turns, on_tool_call=on_tool_call
    )
    print(result)
    return result
...
def cmd_memory_agent(
    ...
    result = service.run_memory_agent(
        prompt, api_key, model, memory_dir=memory_dir, max_turns=max_turns, on_memory_op=on_memory_op
    )
    print(result)
    return result
...
def cmd_embed_file(path: str, model: str = "voyage-3.5", input_type: str = "document"):
    ...
    try:
        ...
        print(f"[ERROR] {e}")
        return
    for line, vec in zip(outcome["lines"], outcome["vecs"], strict=False):
